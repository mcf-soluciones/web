...
async function computeYearCashflow(yyyy, currentMm) {
  ...
  // Free Cash Flow (FCFF, unlevered) = CFO + CFI. Cash generated by the
  // business before any financing decisions. Equivalent to: cash inflows
  // − operating payments − tax payments − capex.
  const fcff = sumMaps([cfo, cfi]);
  ...
  return {
    ...
    fcff: { by_month: fcff, ytd: sumYtd(fcff, currentMm) },
