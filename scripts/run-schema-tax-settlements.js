/**
 * Applies scripts/schema-tax-settlements.sql to Turso. Safe to re-run — every
 * statement uses CREATE ... IF NOT EXISTS, so already-applied changes
 * are no-ops.
 *
 * Usage:
 *   node scripts/run-schema-tax-settlements.js
 */
...
async function main() {
  const sqlPath = path.join(__dirname, 'schema-tax-settlements.sql');
